...
export async function deployContracts(owner, user1, user2, user3) {
    ...
    const raacToken = await deployContract('RAACToken', [owner.address, TAX_RATE, BURN_RATE]);
   
    const veRAACToken = await deployContract('veRAACToken', [raacToken.target]);
    const releaseOrchestrator = await deployContract('RAACReleaseOrchestrator', [raacToken.target]);
    ...
    // Deploy NFT
    const nft = await deployContract('RAACNFT', [crvUSD.target, housePrices.target, owner.address]);
    
    // Deploy Pool Tokens
    const rToken = await deployContract('RToken', ["RToken", "RT", owner.address, crvUSD.target]);
    const debtToken = await deployContract('DebtToken', ["DebtToken", "DT", owner.address]);
    ...
    const feeCollector = await deployContract('FeeCollector', [raacToken.target, veRAACToken.target, treasury.target, repairFund.target, owner.address]);

    // Deploy Stability Pool and DEToken
    const deToken = await deployContract('DEToken', ['DEToken', 'DEToken', owner.address, rToken.target]);
    ...
    await feeCollector.grantRole(await feeCollector.EMERGENCY_ROLE(), owner.address);

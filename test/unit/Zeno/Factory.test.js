...
describe("Test creation of Auctions and ZENOs with Factories", function () {
	...
	it("Should allow creating a new entire auction", async function () {
		...
		const auctionState = await auction.state();

		const {
			endTime: stateEndTime,
			startTime: stateStartTime,
			startingPrice: stateStartingPrice,
			reservePrice: stateReservePrice,
			totalAllocated: stateTotalAllocated,
			totalRemaining: stateTotalRemaining
		} = auctionState;

		expect(stateStartTime).to.equal(auctionStartTime);
		expect(stateEndTime).to.equal(auctionEndTime);
		expect(stateStartingPrice).to.equal(startingPrice);
		expect(stateReservePrice).to.equal(reservePrice);
		expect(stateTotalAllocated).to.equal(totalZENOAllocated);
		expect(stateTotalRemaining).to.equal(totalZENORemaining);

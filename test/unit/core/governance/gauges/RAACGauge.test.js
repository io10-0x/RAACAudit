...
describe("RAACGauge", () => {
  ...
  beforeEach(async () => {
    ...
    await gaugeController.grantRole(
      await gaugeController.GAUGE_ADMIN(),
      owner.address
    );
  ...
  describe("Initialization", () => {
    it("should initialize with correct state", async () => {
      expect(await raacGauge.rewardToken()).to.equal(
        await rewardToken.getAddress()
      );
      expect(await raacGauge.stakingToken()).to.equal(
        await veRAACToken.getAddress()
      );
    ...
    it("should set correct initial parameters", async () => {
      ...
      console.log(votingPeriod[3].toString());
  ...
  describe("Period Management", () => {
    ...
    it("should enforce period boundaries", async () => {
      ...
      await expect(raacGauge.updatePeriod()).to.be.revertedWithCustomError(
        raacGauge,
        "PeriodNotElapsed"
      );

...
describe("BoostController", () => {
    ...
    beforeEach(async () => {
        ...
        await boostController.connect(manager).modifySupportedPool(mockPool.getAddress(), true);
    ...
    describe("Pool Management", () => {
        ...
        it("should remove supported pool", async () => {
            await expect(boostController.connect(manager).modifySupportedPool(mockPool.getAddress(), false))
        ...
        it("should revert when non-manager tries to add pool", async () => {
            await expect(
                boostController.connect(user1).modifySupportedPool(mockPool.getAddress(), true)
    ...
    describe("Integration Scenarios", () => {
        it("should handle multiple users and pools correctly", async () => {
            const pools = await Promise.all([1, 2, 3].map(async () => {
                ...
                await boostController.connect(manager).modifySupportedPool(pool.getAddress(), true);
                return pool;
            }));

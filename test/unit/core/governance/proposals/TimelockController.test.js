...
describe("TimelockController", () => {
    ...
    describe("Operation Execution with ETH", () => {
        let targets;
        let values;
        let calldatas;
        
        beforeEach(async () => {
            // this test contract has a receiveEth function that can receive ETH
            const TimelockTestTarget = await ethers.getContractFactory("TimelockTestTarget");
            testTarget = await TimelockTestTarget.deploy();
            await testTarget.waitForDeployment();
            
            targets = [await testTarget.getAddress(), await testTarget.getAddress()];
            values = [ethers.parseEther("1.0"), ethers.parseEther("2.0")]; // 1 ETH and 2 ETH
            calldatas = [
                testTarget.interface.encodeFunctionData("receiveEth"), 
                testTarget.interface.encodeFunctionData("receiveEth")
            ];
        });

        it("should forward ETH correctly during execution", async () => {
            // Schedule operation
            await timelock.connect(proposer).scheduleBatch(
                targets,
                values,
            ...
            const initialTargetBalance = await ethers.provider.getBalance(await testTarget.getAddress());
            const initialExecutorBalance = await ethers.provider.getBalance(await executor.getAddress());
            ...
            await expect(
                timelock.connect(executor).executeBatch(
                    ...
                    ethers.ZeroHash,
                    ethers.id("ETH_OP"),
                    { value: ethers.parseEther("3.0") } // Total ETH needed
                )
            ).to.emit(timelock, "OperationExecuted");

            // Esure final balance
            const finalExecutorBalance = await ethers.provider.getBalance(await executor.getAddress());
            expect(finalExecutorBalance).to.lt(initialExecutorBalance);
            const finalTargetBalance = await ethers.provider.getBalance(await testTarget.getAddress());
            expect(finalTargetBalance).to.gt(initialTargetBalance);
            expect(finalTargetBalance - initialTargetBalance).to.equal(ethers.parseEther("3.0"));
        });

        it("should revert if insufficient ETH is provided", async () => {
            await timelock.connect(proposer).scheduleBatch(
                targets,
                values,
                calldatas,
            ...
            await expect(
                timelock.connect(executor).executeBatch(
                    ...
                    ethers.ZeroHash,
                    ethers.id("ETH_OP"),
                    { value: ethers.parseEther("2.0") } // Not enough ETH
                )
            ).to.be.reverted;
        });
    });

...
describe("RAACToken", () => {
  ...
  describe("Tax Calculation", () => {
    it("should calculate and distribute taxes correctly", async () => {
      ...
      await raacToken
        .connect(users[0])
        .transfer(users[1].address, transferAmount);

      // Verify tax distribution
      expect(await raacToken.balanceOf(feeCollector.target)).to.equal(
        expectedTax
      );
      expect(await raacToken.totalSupply()).to.equal(
        initialSupply - expectedBurn
      );

      // Verify recipient received correct amount
      const expectedRecipientAmount =
        transferAmount - expectedTax - expectedBurn;
      expect(await raacToken.balanceOf(users[1].address)).to.equal(
    ...
    it("feecollector doesnt get full fees from user burns", async () => {
      ...
      i need to explain this bug first before we get into it
        so this is the _burn function 
    ...
    function burn(uint256 amount) external {
        ...
        and as you know with any normal erc20, _burn will call _update and the RAACtoken overrides the _update function. See below:
    ...
    I have explained most of what this function does in RAACToken.sol. what you need to keep in mind is that in the if block, it says that if the from or to address is the zero address, then the tax is skipped and the normal ERC20 _update is called.

    As you know, with _burn, _update is called with the to address being the zero address so whenever anyone calls burn function in RAACToken, the tax mechanism in _update is skipped. When a user calls burn, first the taxamount is calculated and taken out as you can see above. Then the remaining amount is sent to _burn and since the to address is the zero address, the tax mechanism in _update is skipped and the remaining amount is sent to the zero address. 

    The next thing that happens is that the tax amount is sent to the fee collector. This is where the bug is. Since it is a transfer function used to send the tax amount to the fee collector, _update is obviously called and this time, the from and to address are not the zero address and if the address is not whitelisted, the tax mechanism is applied. So what happens is that as you see above, some of the taxAmount that was supposed to be sent to the fee collector is burnt and this reduces the amount of tax that the fee collector gets which is a key bug.
  ...
  describe("Whitelist Functionality", () => {
    it("should handle whitelist operations correctly", async () => {
      ...
      await raacToken
        .connect(users[0])
        .transfer(whitelistedContract, transferAmount);

      // Verify no tax was taken
      expect(await raacToken.balanceOf(whitelistedContract)).to.equal(
        transferAmount
      );
    });
  });
  ...
  describe("Integration with veRAACToken", () => {
    it("should handle veRAACToken integration correctly", async () => {
      ...
      expect(await raacToken.balanceOf(veRAACToken.target)).to.equal(
        lockAmount
      );
    });
  });
  ...
  describe("Tax Rate Protection Mechanisms", () => {
    async function incrementallySetRate(token, targetRate, isSwapRate = true) {
      ...
      const setRate = async (rate) =>
        isSwapRate
          ? await token.setSwapTaxRate(rate)
          : await token.setBurnTaxRate(rate);
      ...
      while (currentRate !== targetRate) {
        ...
        const effectiveMaxChange =
          maxChange === 0n && currentRate > targetRate ? 1n : maxChange;
        ...
        if (currentRate < targetRate) {
          ...
          nextRate =
            targetRate <= maxAllowedIncrease ? targetRate : maxAllowedIncrease;
        } else {
          // Decreasing
          const maxAllowedDecrease = effectiveMaxChange;
          const rateDecrease =
            currentRate - targetRate > maxAllowedDecrease
              ? maxAllowedDecrease
      ...
      if (finalRate !== targetRate) {
        throw new Error(
          `Failed to reach target rate. Current: ${finalRate}, Target: ${targetRate}`
        );
      }
    }
    ...
    it("should verify tax calculations with different rates", async function () {
      ...
      for (const rates of testRates) {
        ...
        if (user0Balance > 0n) {
          await raacToken
            .connect(users[0])
            .transfer(owner.address, user0Balance);
        }
        ...
        // Verify balances
        expect(await raacToken.balanceOf(users[1].address)).to.equal(
          initialBalances.recipient + expectedRecipient,
          "Recipient balance incorrect"
        );
        expect(await raacToken.balanceOf(feeCollector.target)).to.equal(
          initialBalances.feeCollector + expectedTax,
          "Fee collector balance incorrect"
        );
        expect(await raacToken.totalSupply()).to.equal(
          initialBalances.supply - expectedBurn,
    ...
    it("should handle high tax rates correctly", async function () {
      ...
      expect(await raacToken.balanceOf(users[1].address)).to.equal(
        expectedRecipient,
        "High tax rate: Recipient balance incorrect"
      );
      expect(await raacToken.totalSupply()).to.equal(
        initialSupply - expectedBurn,
        "High tax rate: Total supply incorrect after burn"
      );
    });
    ...
    it("should enforce increment limits for decreasing tax rates", async function () {
      ...
      await expect(
        raacToken.setSwapTaxRate(targetRate)
      ).to.be.revertedWithCustomError(
        raacToken,
        "TaxRateChangeExceedsAllowedIncrement"
      );
    ...
    it("should enforce BASE_INCREMENT_LIMIT for tax rate increment limit", async function () {
      ...
      await expect(
        raacToken.setTaxRateIncrementLimit(1001n)
      ).to.be.revertedWithCustomError(
        raacToken,
        "IncrementLimitExceedsBaseLimit"
      );
    });

    it("should handle zero address checks properly", async function () {
      await expect(
        raacToken.setMinter(ethers.ZeroAddress)
      ).to.be.revertedWithCustomError(raacToken, "InvalidAddress");

      await expect(
        raacToken.manageWhitelist(ethers.ZeroAddress, true)
      ).to.be.revertedWithCustomError(raacToken, "CannotWhitelistZeroAddress");

      await expect(
        raacToken.manageWhitelist(ethers.ZeroAddress, false)
      ).to.be.revertedWithCustomError(
        raacToken,
        "CannotRemoveZeroAddressFromWhitelist"
      );
    });
    ...
    it("should handle whitelist duplicate operations", async function () {
      ...
      // Try to add again
      await expect(
        raacToken.manageWhitelist(testAddress, true)
      ...
      // Try to remove again
      await expect(
        raacToken.manageWhitelist(testAddress, false)
...
async function setTaxRatesIncrementally(token, targetSwapRate, targetBurnRate) {
  ...
  // Adjust both rates
  await adjustRate(currentSwapRate, targetSwapRate, (rate) =>
    token.setSwapTaxRate(rate)
  );
  await adjustRate(currentBurnRate, targetBurnRate, (rate) =>
    token.setBurnTaxRate(rate)
  );
}

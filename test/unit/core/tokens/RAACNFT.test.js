...
describe("RAACNFT", () => {
    ...
    describe("Initialization", () => {
        ...
        it("should revert with zero addresses", async () => {
            const RAACNFT = await ethers.getContractFactory("RAACNFT");
            await expect(
                RAACNFT.deploy(ethers.ZeroAddress, await housePrices.getAddress(), owner.address)
            ).to.be.revertedWithCustomError(RAACNFT, "RAACNFT__InvalidAddress");

            await expect(
                RAACNFT.deploy(await crvUSD.getAddress(), ethers.ZeroAddress, owner.address)
            ).to.be.revertedWithCustomError(RAACNFT, "RAACNFT__InvalidAddress");

            await expect(
                RAACNFT.deploy(await crvUSD.getAddress(), await housePrices.getAddress(), ethers.ZeroAddress)
    ...
    describe("Minting", () => {
        ...
        it("should revert with insufficient funds", async () => {
            ...
            ).to.be.revertedWithCustomError(raacNFT, "RAACNFT__InsufficientFundsMint");
        });

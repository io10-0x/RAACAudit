...
describe("FeeCollector", function () {
  ...
  beforeEach(async function () {
    [owner, user1, user2, treasury, newTreasury, repairFund, emergencyAdmin] =
    ...
    await raacToken
      .connect(user1)
      .approve(await veRAACToken.getAddress(), ethers.MaxUint256);
    await raacToken
      .connect(user2)
      .approve(await veRAACToken.getAddress(), ethers.MaxUint256);
  ...
  describe("Constructor & Initial Setup", function () {
    it("should set the correct initial values", async function () {
      expect(await feeCollector.raacToken()).to.equal(
        await raacToken.getAddress()
      );
      expect(await feeCollector.veRAACToken()).to.equal(
        await veRAACToken.getAddress()
      );
  ...
  describe("Fee Collection and Distribution", function () {
    it("should distribute fees correctly between stakeholders", async function () {
      ...
      const initialTreasuryBalance = await raacToken.balanceOf(
        treasury.address
      );
      const initialRepairFundBalance = await raacToken.balanceOf(
        repairFund.address
      );
      ...
      const netMultiplier =
        BigInt(BASIS_POINTS - taxRate) / BigInt(BASIS_POINTS);
      ...
      const finalRepairFundBalance = await raacToken.balanceOf(
        repairFund.address
      );
    ...
    it("should emit FeeDistributed event with correct amounts", async function () {
      ...
      const veRAACShare =
        (totalFees * BigInt(feeType0.veRAACShare)) / BigInt(BASIS_POINTS);
      const burnShare =
        (totalFees * BigInt(feeType0.burnShare)) / BigInt(BASIS_POINTS);
      const repairShare =
        (totalFees * BigInt(feeType0.repairShare)) / BigInt(BASIS_POINTS);
      const treasuryShare =
        (totalFees * BigInt(feeType0.treasuryShare)) / BigInt(BASIS_POINTS);
      ...
      const veRAACShareNet =
        (veRAACShare * netMultiplier) / BigInt(BASIS_POINTS);
      const burnShareNet = (burnShare * netMultiplier) / BigInt(BASIS_POINTS);
      const repairShareNet =
        (repairShare * netMultiplier) / BigInt(BASIS_POINTS);
      const treasuryShareNet =
        (treasuryShare * netMultiplier) / BigInt(BASIS_POINTS);
    ...
    it("should not allow distribution when paused", async function () {
      await feeCollector
        .connect(owner)
        .grantRole(await feeCollector.EMERGENCY_ROLE(), owner.address);
    ...
    it("should not allow distribution by non-distributor", async function () {
      await expect(
        feeCollector.connect(user1).distributeCollectedFees()
    ...
    it("should allow users to claim rewards", async function () {
      ...
      expect(await raacToken.balanceOf(user1.address)).to.be.gt(initialBalance); //c so all they check is that the user's balance is greater than it was before which is definitely not a good enough check as I am going to prove now
    });
  ...
  describe("Fee Collection", function () {
    ...
    it("should collect fees correctly", async function () {
      ...
      console.log(
        "Initial Collected Fees:",
        initialCollectedFees.protocolFees.toString()
      );

      // Calculate expected amounts
      const taxRate = SWAP_TAX_RATE + BURN_TAX_RATE; // 150 basis points (1.5%)
      const expectedNet =
        (amount * BigInt(BASIS_POINTS - taxRate)) / BigInt(BASIS_POINTS);
      ...
      console.log(
        ...
        finalCollectedFees.protocolFees.toString()
      );

      // Calculate and verify changes
      const balanceChange = finalBalance - initialBalance;
      const collectedFeesChange =
        finalCollectedFees.protocolFees - initialCollectedFees.protocolFees;
      ...
      expect(amount).to.equal(
        ...
        "Gross amount should be 100 ETH"
      );
      expect(expectedNet).to.equal(
        98500000000000000000n,
        "Net amount should be 98.5 ETH"
      );
      expect(taxAmount).to.equal(
        1500000000000000000n,
        "Tax amount should be 1.5 ETH"
      );
    ...
    it("should revert on invalid fee type", async function () {
      await expect(
        feeCollector.connect(user1).collectFee(ethers.parseEther("100"), 8)
      ).to.be.revertedWithCustomError(feeCollector, "InvalidFeeType");
    });

    it("should revert on zero amount", async function () {
      await expect(
        feeCollector.connect(user1).collectFee(0, 0)
  ...
  describe("Emergency Controls", function () {
    beforeEach(async function () {
      ...
      await raacToken
        .connect(user1)
        .transfer(feeCollector.target, ethers.parseEther("100"));
    });
    ...
    it("should allow emergency withdrawal by admin", async function () {
      ...
      const secondTaxAmount =
        (remainingAfterFirstTax * BigInt(taxRate)) / BigInt(10000);
    ...
    it("should not allow emergency withdrawal when not paused", async function () {
      await expect(
        feeCollector.connect(user1).emergencyWithdraw(raacToken.target)
    ...
    it("should not allow non-EMERGENCY_ROLE to pause", async function () {
      await expect(
        feeCollector.connect(user1).pause()
      ).to.be.revertedWithCustomError(feeCollector, "UnauthorizedCaller");
    });
    ...
    it("should not allow non-EMERGENCY_ROLE to unpause", async function () {
      await feeCollector.connect(owner).pause();
      await expect(
        feeCollector.connect(user1).unpause()
      ).to.be.revertedWithCustomError(feeCollector, "UnauthorizedCaller");
    });
  });
  ...
  describe("Fee Type Management", function () {
    it("should allow fee type update by manager", async function () {
      await feeCollector
        .connect(owner)
        .grantRole(await feeCollector.FEE_MANAGER_ROLE(), owner.address);
    ...
    it("should revert on invalid fee shares total", async function () {
      ...
      await expect(
        feeCollector.connect(owner).updateFeeType(0, invalidFeeType)
  ...
  describe("FeeCollector Math Functions", function () {
    ...
    describe("Fee Distribution Calculations", () => {
      ...
      it("should handle dust amounts correctly", async () => {
        ...
        await feeCollector
          .connect(owner)
          .collectFee(ethers.parseEther("100.123456789"), 0);
    ...
    describe("Gas Optimization", () => {
      it("should be gas efficient for distribution calculations", async () => {
        ...
        const tx =
          await feeCollector.distributeCollectedFees.populateTransaction();

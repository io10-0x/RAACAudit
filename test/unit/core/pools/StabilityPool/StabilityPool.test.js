...
describe("StabilityPool", function () {
  ...
  describe("Core Functionality", function () {
    describe("Deposits", function () {
      beforeEach(async function () {
        ...
        await rToken
          .connect(user2)
          .approve(stabilityPool.target, depositAmount);
      });
      ...
      it("test precision loss possibility", async function () {
        //c for testing purposes.

        //c so far precision loss isnt really much of a thing but i will keep testing
        const initialAmount = ethers.parseEther("2000000");
        await stabilityPool.connect(user2).deposit(initialAmount);

        const rcrvusdamount = await stabilityPool.calculateRcrvUSDAmount(
          initialAmount
        );
        console.log("rcrvusdamount", rcrvusdamount.toString());
      ...
      it("frontrun opportunity due to stepwise jump", async function () {
        ...
        const depositAmount = ethers.parseEther("102.75784748477");
        await stabilityPool.connect(user2).deposit(depositAmount);
        ...
        const frontrunAmount = ethers.parseEther("50000");
        await stabilityPool.connect(user1).deposit(frontrunAmount);

        const user2balprewithdraw = await raacToken.balanceOf(user2.address);
        console.log("user2balprewithdraw", user2balprewithdraw.toString());

        const user1balprewithdraw = await raacToken.balanceOf(user1.address);
        console.log("user1balprewithdraw", user1balprewithdraw.toString());
      ...
      it("precision loss checks", async function () {
        //c for testing purposes.
        ...
        const user1pendingReward = await stabilityPool.getPendingRewards(
          user1.address
        );
        console.log("user1pendingReward", user1pendingReward.toString());
        ...
        const pendingrewardsum = user2pendingReward + user1pendingReward;
        console.log("pendingrewardsum", pendingrewardsum.toString());

        assert(pendingrewardsum == stabilitypoolbal);
      });
    });
    ...
    describe("RAAC Rewards", function () {
      beforeEach(async function () {
        ...
        await rToken
          .connect(user1)
          .approve(stabilityPool.target, depositAmount1);
        ...
        await rToken
          .connect(user2)
          .approve(stabilityPool.target, depositAmount2);
      });
    ...
    describe("Dust Handling", function () {
      beforeEach(async function () {
        ...
        await rToken
          .connect(user1)
          .approve(stabilityPool.target, initialAmount);
      });
      ...
      it("should handle dust amounts correctly in withdrawals", async function () {
        ...
        const remainingDeposit = await stabilityPool.getUserDeposit(
          user1.address
        );
  ...
  describe("Market Management", function () {
    ...
    it("should add market correctly", async function () {
      ...
      expect(await stabilityPool.marketAllocations(user1.address)).to.equal(
        100
      );
    });
  ...
  describe("Access Control", function () {
    ...
    it("managers should be removable", async function () {
      //c for testing purposes
      await stabilityPool.addManager(user1.address, 100);
      expect(await stabilityPool.getManager(user1.address)).to.be.true;

      await stabilityPool.addManager(user2.address, 100);
      expect(await stabilityPool.getManager(user1.address)).to.be.true;

      await stabilityPool.addManager(user3.address, 100);
      expect(await stabilityPool.getManager(user1.address)).to.be.true;

      const getmanagers1 = await stabilityPool.getManagers();
      console.log("getmanagers", getmanagers1);

      await stabilityPool.removeManager(user2.address);
      expect(await stabilityPool.getManager(user2.address)).to.be.false;

      const getmanagers = await stabilityPool.getManagers();
      console.log("getmanagers", getmanagers);
    });

    it("should enforce manager allocation limits", async function () {
      await stabilityPool.addManager(user1.address, 100);
      const allocation = await stabilityPool.getManagerAllocation(
        user1.address
      );
    ...
    it("should update manager allocation", async function () {
      ...
      expect(await stabilityPool.getManagerAllocation(user1.address)).to.equal(
        200
      );
    });
  ...
  describe("RAACMinter Integration", function () {
    ...
    it("should handle minter pausing correctly", async function () {
      ...
      const rewardsBefore = await stabilityPool.calculateRaacRewards(
        user1.address
      );
      ...
      const rewardsAfter = await stabilityPool.calculateRaacRewards(
        user1.address
      );
  ...
  describe("Market Management Edge Cases", function () {
    ...
    it("should handle market removal and reallocation correctly", async function () {
      ...
      expect(await stabilityPool.marketAllocations(user3.address)).to.equal(
        allocation
      );

      // Check total allocation remains the same
      expect(await stabilityPool.totalAllocation()).to.equal(
        initialTotalAllocation
      );
    });
  });
  ...
  describe("State Consistency", function () {
    it("should maintain correct state after multiple operations", async function () {
      ...
      await rToken
        .connect(user1)
        .approve(stabilityPool.target, additionalAmount);
